...
class BinaryTree {
   ...
   public:
    ...
    BinaryTree *predecessor(BinaryTree *node) {
        ...
        if (node->left != NULL) return treeMaximum(node->left);
    ...
    bool check(BinaryTree *root, int min, int max) {
        ...
        return check(root->left, min, root->getValue()) &&
               check(root->right, root->getValue(), max);
    }
    ...
    void printValuesInRange(BinaryTree *node, int i, int j) {
        ...
        if (node->getValue() > i) printValuesInRange(node->left, i, j);

        if (node->getValue() >= i && node->getValue() <= j)
            cout << node->getValue() << " ";

        if (node->getValue() < j) printValuesInRange(node->right, i, j);
    }
...
int main(int argc, char *argv[]) {
    ...
    printf("Tree Successor   of Root = %d is %d \n", Root->getValue(),
           Root->successor(Root)->getValue());
    printf("Tree Predecessor of Root = %d is %d \n", Root->getValue(),
           Root->predecessor(Root)->getValue());
